                            ...
                            <?php
                              ...
                              if(empty($seletor) || empty($token)){
                                ...
                                if(ctype_xdigit($selector) !== false && ctype_xdigit($token) !== false) {
                                    ?>
                                <form  action="../process/auth/reset-pwd.inc.php" method="post" class="sign-form">
                                <div class="form-group">
                                        <?php if(isset($_GET['message'])): $error = $_GET['message']; ?>
                                                <div>
                                                    <h5 class="text-success"><?php echo $error ?></h5>
                                                </div>
                                        <?php endif ?>

                                        <input type="hidden" name="selector" value='<?=$selector?>'>
                                        <input type="hidden" name="selector" value='<?=$validator?>'>

                                        <input type="text" name="username" placeholder="Enter your username" required><br>
                                        <input type="password" name="pwd" placeholder="New password" required><br>
                                        <input type="password" name="new-pwd" placeholder="Repeate new password" required><br>
                                        <button class="axil-button button-rounded" type="submit" name="reset-btn">Reset Password</button>
