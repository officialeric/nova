...
if(isset($_POST['img_submit']) && isset($_FILES['image'])) {
    ...
    if(!$error) {
        if($img_size > 5000000) {
            header("location: ../../user-profile.php?user_id=".$id."&error=Image is too large!!!!");
        }else {
            ...
            if(in_array($img_ex_lc, $allowed_exs)) {
                ...
                if($result) {
                    header("location: ../../user-profile.php?user_id=".$id."&msg=Image uploaded successfully!");
                }else {
                    header("location: ../../user-profile.php?user_id=".$id."&error=Image failed to upload!");
                }

            }else {
                header("location: ../../user-profile.php?user_id=".$id."&error=Image format not supported!");
            }
        }
    }
